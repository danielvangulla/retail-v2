...
class PrinterPiutangServices extends BasePrinter implements ShouldQueue
{
    ...
    public function startPrint(Object $data)
    {
        ...
        $this->horizontalLine($print);
        $print->selectPrintMode(Printer::MODE_DOUBLE_WIDTH);
        $print->text("  Pembayaran Tertunda\n");
        $print->selectPrintMode();

        $print->feed();
        $this->printSum($print, "PIUTANG by {$data->trx->piutang->name} Rp.", $data->trx->bayar);
        $print->feed();
