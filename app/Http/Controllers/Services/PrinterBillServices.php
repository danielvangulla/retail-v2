...
class PrinterBillServices extends BasePrinter
{
    public function printBill($printer, $transaksi_id, $isPayment)
    {
        $trx = Transaksi::with(['details' => function ($query) {
    ...
    public function startPrint(Object $data)
    {
        ...
        if ($data->is_payment) {
            $this->printPayment($print, $data->trx);
        } else {
            $this->printRequestBill($print);
        }
    ...
    private function printRequestBill($print)
    {
        $print->setFont(Printer::FONT_B);
        $print->text("  ** ini bukan bukti pembayaran.\n");
        $print->setFont(Printer::FONT_A);
    }

    private function printPayment($print, $trx)
    {
        $print->selectPrintMode(Printer::MODE_DOUBLE_WIDTH);
